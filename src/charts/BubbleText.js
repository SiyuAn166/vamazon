...
function drawLegend(svg, color) {
  ...
  const legendItems = legend
    ...
    .on("mouseenter", (e, d) => {
      const nonHoveringText = svg
        .selectAll(".bubble-text-item")
        .filter(function () {
          return this.getAttribute("grp") !== d;
        });
      nonHoveringText.transition().duration(300).attr("opacity", 0.1);
    })
    .on("mouseleave", (e, d) => {
      const nonHoveringText = svg
        .selectAll(".bubble-text-item")
      ...
      nonHoveringText.transition().duration(300).attr("opacity", 1);
    })
    .on("click", (e, d) => {
      const hoveringText = svg
        .selectAll(".bubble-text-item")
        .filter(function () {
          return this.getAttribute("grp") === d;
        });
...
function drawShadow(svg) {
  ...
  svg.selectAll(".bubble-text-item").attr("filter", "url(#text-shadow)");
}
...
const BubbleText = ({ data }) => {
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force(
        "charge",
        d3.forceManyBody().strength((d, i) => (i ? 0 : 3))
      )
      .alpha(1)
      .alphaTarget(0.3)
      .velocityDecay(0.1);

    const textGroups = svg.selectAll("g").data(data).enter().append("g");
    ...
    function onMouseEnter(event, d) {
      ...
      hoverOn.transition().duration(300).style("opacity", 1);
    }
